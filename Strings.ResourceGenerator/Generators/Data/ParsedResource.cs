...
internal class ParsedResource
{
    ...
    public ParsedResource(ResourceParser parser, ResourceString resourceString)
    {
        ...
        if (parser.StringType != StringType.Simple)
        {
            ...
            getter = parser.StringType == StringType.Format
                ? $"           <getter_operator> Format(\"<string>\", {string.Join(", ", parser.SignatureParameters)});"
                : parser.StringType == StringType.Interpolation
                    ? $"           <getter_operator> $\"<string>\";"
                    : $"           <getter_operator> \"<string>\";";
